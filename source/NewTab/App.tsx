...
import type { HomebaseSettings, HomebaseThemeId, HotLink, WeatherLocation } from "../types/storage";
...
function normalizeUrl(value: string): string {
	...
	const urlWithProtocol = /^[a-z][a-z\d+.-]*:/i.test(trimmedValue) ? trimmedValue : `https://${trimmedValue}`;
...
const App: FC = () => {
	const [homebase, setHomebase] = useState<HomebaseSettings>(defaultHomebaseSettings);
	...
	const deleteLink = (linkId: string): void => {
		setHomebase((currentHomebase): HomebaseSettings => {
			return {
				...currentHomebase,
				hotLinks: currentHomebase.hotLinks.filter((link): boolean => link.id !== linkId),
			};
		});
	};
	...
	const updateWeatherLocation = (weatherLocation: WeatherLocation | null): void => {
		setHomebase((currentHomebase): HomebaseSettings => {
			return {
				...currentHomebase,
				weatherLocation,
	...
	const pageClassName = `${styles.page} ${isThemeChanging ? styles.themeChanging : ""}`;

	return (
		<div className={pageClassName} data-theme={homebase.themeId}>
			<div className={styles.shell}>
				<ThemePicker selectedThemeId={homebase.themeId} onThemeChange={updateTheme} />
				<div className={styles.topbar}>
					<Header />
					<WeatherWidget location={homebase.weatherLocation} onLocationChange={updateWeatherLocation} />
				</div>
