...
import { AlertTriangle, Cloud, CloudRain, CloudSnow, CloudSun, Search, Sun, X, Zap } from "lucide-react";
import type { WeatherLocation, WeatherReport } from "../../types/storage";
import { fetchWeatherReport, formatLocation, searchWeatherLocations } from "../utils/weather";
...
export const WeatherWidget: FC<WeatherWidgetProps> = ({ location, onLocationChange }) => {
	...
	return (
		<>
			<button type="button" className={styles.weatherWidget} onClick={(): void => setIsPickerOpen(true)} aria-label="Set weather city">
				<span className={styles.weatherIcon}>{getWeatherIcon(weatherReport?.condition)}</span>
				<span className={styles.weatherContent}>
					...
					{weatherReport && location ? (
						<>
							<span className={styles.weatherTemp}>{weatherReport.temperature}&deg;F</span>
			...
			{isPickerOpen && (
				...
				>
					<section className={styles.weatherModal} role="dialog" aria-modal="true" aria-labelledby="weather-modal-title">
						<div className={styles.modalHeader}>
							<div>
								<p className={styles.kicker}>Weather location</p>
								<h2 id="weather-modal-title">Pick your city</h2>
							</div>
							<button type="button" className={styles.iconButton} onClick={(): void => setIsPickerOpen(false)} aria-label="Close weather picker" title="Close">
							...
							<div className={styles.cityPicker}>
								<form className={styles.citySearch} onSubmit={handleSearch}>
									<label>
										...
										<input type="search" value={query} onChange={(event): void => setQuery(event.target.value)} placeholder="Richmond" autoComplete="off" />
									</label>
									<button type="submit" className={styles.iconButton} aria-label="Search cities" title="Search" disabled={isSearching}>
										<Search size={18} aria-hidden="true" />
									</button>
								</form>

								{searchError && <p className={styles.formError}>{searchError}</p>}

								<div className={styles.cityResults}>
									{results.map((result) => (
										<button key={`${result.id}-${result.latitude}`} type="button" onClick={(): void => selectLocation(result)}>
								...
								{location && (
									<button type="button" className={styles.clearLocationButton} onClick={(): void => onLocationChange(null)}>
										Clear saved city
									</button>
								)}
							</div>
