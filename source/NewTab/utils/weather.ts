...
import type { CachedDailyWeather, WeatherLocation, WeatherReport } from "../../types/storage";
...
export function getWeatherDescription(weatherCode?: number): string {
	...
	if (weatherCode === 61 || weatherCode === 63 || weatherCode === 65 || weatherCode === 66 || weatherCode === 67 || weatherCode === 80 || weatherCode === 81 || weatherCode === 82) {
		return "Rain";
	}
...
export async function searchWeatherLocations(searchTerm: string): Promise<WeatherLocation[]> {
...
function isValidDailyWeatherCache(value: unknown): value is CachedDailyWeather {
	...
	return typeof cache.locationId === "number" && typeof cache.forecastDate === "string" && typeof cache.high === "number" && typeof cache.low === "number" && typeof cache.fetchedAt === "number";
}

async function getCachedDailyWeather(location: WeatherLocation): Promise<DailyWeather | null> {
...
async function fetchFreshDailyWeather(location: WeatherLocation): Promise<DailyWeather> {
	const url = new URL(forecastUrl);
	url.searchParams.set("latitude", String(location.latitude));
	url.searchParams.set("longitude", String(location.longitude));
	...
	if (typeof daily?.temperature_2m_max?.[0] !== "number" || typeof daily.temperature_2m_min?.[0] !== "number") {
...
async function getDailyWeather(location: WeatherLocation): Promise<DailyWeather> {
...
export async function fetchWeatherReport(location: WeatherLocation): Promise<WeatherReport> {
	const [currentWeather, dailyWeather] = await Promise.all([fetchCurrentWeather(location), getDailyWeather(location)]);
